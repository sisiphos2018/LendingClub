...
Needed <- c("DescTools","choroplethrMaps","choroplethr","car","corrplot",
...
loan_raw$pub_rec_bankruptcies=as.numeric(loan_raw$pub_rec_bankruptcies)
loan_raw$tax_liens=as.numeric(loan_raw$tax_liens)
...
loan_raw$delinq_2yrs=as.numeric(loan_raw$delinq_2yrs)
loan_raw$collections_12_mths_ex_med=as.numeric(loan_raw$collections_12_mths_ex_med)
...
write.csv(loanNewNUM,file="Numloan.csv")

loanChar=Filter(is.character,loan_raw)
write.csv(loanChar,file="CHARloan.csv")
###################################
...
loanNewNUM=select(loanNewNUM,-emp_lengthAsNum  )
...
#save trsnsformed data
save(loanNewNUM,file="NoMonthHaveNA.Rdata")

## Create and save subset of rejection data
set.seed(10)
loan_sample <- sample_n(LoanNoNA, size = 100000)
save(loan_sample, file = 'loan_sampleNoNaAndmonth.RData')
